...
public class WaveletMapper
{
	...
	public static int [] forward_transform(int [] src, int xdim)
	{
		...
		// Assumes image dimension is even.
		int min  = xdim;
		// Could get stuck in a loop if dimension is a power of 2.
		while(min % 2 == 0)
			min /= 2;
		//min *= 2;
	...
	*/
	...
	public static void inverse_daub4(int [] src, int offset, int xdim, int min)
	{
		double [] c       = {0.4829629, 0.8365163, 0.2241438, -0.129409};
		int    [] buffer = new int[xdim];
		int i, j;
	...
	public static int [] inverse_transform(int [] src, int xdim)
	{
	    ...
	    for(int i = 0; i < xdim / 2; i++)  
	    {
	        ...
	        for(int j = min; j <= ydim; j *= 2)
	        {
	        	inverse_daub4(buffer, offset, j, min);
	        }
	    }
	    ...
	    for(int i = 0; i < ydim ; i++)
	    {
	    	...
	    	for(int j = min; j <= xdim; j *= 2)
	    	{
	    		inverse_daub4(dst, offset, j, min);	
	    	}
	    }
